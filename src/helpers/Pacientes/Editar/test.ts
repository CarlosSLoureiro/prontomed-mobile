...
describe('helpers > Pacientes > Editar', () => {
  let repository: PacientesRepositoryInterface;
  let editarSpy: jest.SpyInstance<any>;
  let helper: EditarPacientesHelper;

  beforeEach(() => {
    repository = new PacientesRepositoryMock();
    editarSpy = jest.spyOn(repository, 'editar');
    helper = new EditarPacientesHelper(repository);
  });

  test('deve editar o paciente com sucesso', async () => {
